  ...
  render () {
    ...
    const drawOptions = options.map((option, i) => {
      return  <div className={`py-3 duration-200 ${i + 1 === options.length ? 'rounded-b-md' : ''} ${newOptionHoverDark} ${newOptionHoverLight} ${newDarkColor} ${newLightColor}`} onClick={() => {this.setState({ selectedOption: option }); setSortCode(option)}}>
                <span className='text-xs font-semibold text-white dark:text-slate-800'>{ option === 'Dijkstra`s' ? 'Dijkstra`s (Weighted)' : option }</span>
              </div>
    })
    ...
    return (
      <div className="relative" ref={this.wrapperRef}>
        ...
        >
            <div className={`h-14 text-md font-semibold dark:hover:text-slate-800 ${focus ? 'dark:text-slate-800 text-white' : 'text-white'} pt-4 ${isDisabled ? 'opacity-25' : ''}`} >{ selectedOption === '' ? 'Select an Algorithm!' : selectedOption === 'Dijkstra`s' ? 'Dijkstra`s (Weighted)' : selectedOption}</div>
            {focus ? 
